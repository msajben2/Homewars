// =====================================================================
// RODINNÁ HRA - HOME WARS (JAVASCRIPT ENGINE - ČASŤ 1/2)
// =====================================================================
...
function aktualizujPanelDielne() {
    ...
    devBtnDiv.innerHTML = '<button onclick="devPridatSurovinyACheaty()" style="background:#8b5cf6; color:#fff; border:none; padding:10px 20px; border-radius:6px; font-weight:bold; cursor:pointer; width:100%;">⚡ DEV CHEAT: Pridať 100 000 Mincí, Zlato, Prízrakov & Suroviny pre Kovanie až po S-Class</button>'; e.appendChild(devBtnDiv);
    ...
    var prizrakCardDiv = document.createElement("div"); prizrakCardDiv.className = "karta cls-PRIZRAK-F"; prizrakCardDiv.innerHTML = vytvorHTMLKarty("Prízrak", "none", "F", 0, 0);
    ...
    Object.keys(MASTER_REGISTRY).forEach(function(t) {
        ...
        if (typeof cardData.repliky !== "object") { var val = cardData.repliky || 0; cardData.repliky = { "F": val }; }
        var wrapper = document.createElement("div"); wrapper.className = "karta-karta-wrapper";
        var cardDiv = document.createElement("div"); cardDiv.className = "karta cls-" + cardData.aktivnaTrieda;
        var realPwr = getRealPower({ n: t, cls: cardData.aktivnaTrieda }); cardDiv.innerHTML = vytvorHTMLKarty(t, realPwr, cardData.aktivnaTrieda, reg.row, reg.p);
        var countsText = reg.isTournamentUnique ? '👑 Turnajový Unikát (Kovanie: 1x Šampión + 2x Prízrak)' : 'F:' + (cardData.repliky["F"] || 0) + ' | E:' + (cardData.repliky["E"] || 0) + ' | D:' + (cardData.repliky["D"] || 0) + ' | C:' + (cardData.repliky["C"] || 0) + ' | B:' + (cardData.repliky["B"] || 0) + ' | A:' + (cardData.repliky["A"] || 0);
...
function vylepsiKartuVoForge(meno, transitionKey, pergamenType) {
    ...
    if (reg.isPrizrak) {
        ...
        if ((countCurrent + countPrizrak) < 3) { ukazOznamenie("⚠️ NEDOSTATOK KARIET", "Potrebuješ 3x rovnocenné karty (alebo Prízrakov)!"); return; }
    }
...
function spustitVideoAnimationKovania(meno, oldCls, nextCls, isSuccess, wasProtected) {
    ...
    vid.onended = function() {
        if (reg.isPrizrak) {
            ...
            var availableReal = t.repliky[oldCls] || 0; var realToDeduct = Math.min(3, availableReal); var prizrakyToDeduct = 3 - realToDeduct;

            if (isSuccess) {
                t.repliky[oldCls] = Math.max(0, availableReal - realToDeduct);
                if (prizrakyToDeduct > 0) inventar.prizraky[oldCls] = Math.max(0, (inventar.prizraky[oldCls] || 0) - prizrakyToDeduct);
                ...
                if (!wasProtected) {
                    if (availableReal > 0) t.repliky[oldCls] = Math.max(0, availableReal - 1); else inventar.prizraky[oldCls] = Math.max(0, (inventar.prizraky[oldCls] || 0) - 1);
...
// =====================================================================
// RODINNÁ HRA - HOME WARS (JAVASCRIPT ENGINE - ČASŤ 2/2)
// =====================================================================
...
function vygenerujSimulaciuTrhu() {
    ...
    if (aktualnaZalozkaTrhu === "trh") {
        var reg = MASTER_REGISTRY["Neviditeľný Mário"]; var realPwr = getRealPower({ n: "Neviditeľný Mário", cls: "E" });
...
function potvrditMulliganAkciu(chceVymenu) {
    ...
    if (chceVymenu && mulliganSelectedIndices.length > 0) {
        ...
        for (var i = 0; i < pocetVymen; i++) { if (p1_active_deck.length > 0) { var nK = p1_active_deck.pop(); p1_draft_hand.push({ n: nK, cls: (inventar.karty[nK] ? inventar.karty[nK].aktivnaTrieda : "F") }); } }
...
function vykresliStraneKnihy() {
    ...
    if (aktualnaStranaKnihy === 1) container.innerHTML = '<h3 style="color:#ffcc00;">📜 KAPITOLA I: ŠANCE DROPINGU</h3><p>Odmeny dostávaš po dokončení zápasu...</p>';
    else if (aktualnaStranaKnihy === 2) container.innerHTML = '<h3 style="color:#ffcc00;">🔨 KAPITOLA II: KOVÁČSKY STROM</h3><p>Na kovanie potrebuješ 3 rovnocenné karty...</p>';
    else if (aktualnaStranaKnihy === 3) container.innerHTML = '<h3 style="color:#ffcc00;">🛠️ KAPITOLA III: SETOVÉ BONUSY</h3><p>Za vykované karty získavaš bonusy...</p>';
    else if (aktualnaStranaKnihy === 4) container.innerHTML = '<h3 style="color:#ffcc00;">⚡ KAPITOLA IV: 20 PLATINOVÝCH KARIET</h3><p>Existuje 20 unikátnych Platinových kariet...</p>';
    else if (aktualnaStranaKnihy === 5) container.innerHTML = '<h3 style="color:#ffcc00;">🛒 KAPITOLA V: TRHOVISKO</h3><p>Aukcie prebiehajú anonymne...</p>';
}
